# TaskRequestBody


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**body** | **string** |  | [optional] [default to undefined]
**createdAt** | **string** |  | [optional] [default to undefined]
**start** | **string** |  | [optional] [default to undefined]
**ending** | **string** |  | [optional] [default to undefined]
**status** | **string** |  | [optional] [default to undefined]
**privacy** | **string** |  | [optional] [default to undefined]
**importance** | **number** |  | [optional] [default to undefined]
**color** | **string** |  | [optional] [default to undefined]
**id** | **string** |  | [optional] [default to undefined]
**clientUuid** | **string** |  | [optional] [default to undefined]
**title** | **string** |  | [optional] [default to undefined]
**repeatTask** | **string** |  | [optional] [default to undefined]
**user** | **string** |  | [optional] [default to undefined]
**pushTemplate** | **string** |  | [optional] [default to undefined]
**userId** | [**User**](User.md) |  | [optional] [default to undefined]
**end** | **string** |  | [optional] [default to undefined]
**edited** | **boolean** |  | [optional] [default to undefined]

## Example

```typescript
import { TaskRequestBody } from './api';

const instance: TaskRequestBody = {
    body,
    createdAt,
    start,
    ending,
    status,
    privacy,
    importance,
    color,
    id,
    clientUuid,
    title,
    repeatTask,
    user,
    pushTemplate,
    userId,
    end,
    edited,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
